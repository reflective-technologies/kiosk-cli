...
func formatAPIError(sb *strings.Builder, err *APIError) {
	sb.WriteString(color(style.Red+style.Bold, "Error: "))
	sb.WriteString(getNarrativeMessage(err))
	sb.WriteString("\n")

	if DevMode {
		sb.WriteString("\n")
		sb.WriteString(color(style.Dim, "--- Debug Info ---\n"))
		sb.WriteString(color(style.Dim, fmt.Sprintf("Status Code: %d\n", err.StatusCode)))
		sb.WriteString(color(style.Dim, fmt.Sprintf("Raw Message: %s\n", err.Message)))
		if err.RawBody != "" && err.RawBody != err.Message {
			sb.WriteString(color(style.Dim, fmt.Sprintf("Raw Body: %s\n", err.RawBody)))
		}
	}
}
...
func formatNetworkError(sb *strings.Builder, err *NetworkError) {
	...
	if DevMode {
		...
		sb.WriteString(color(style.Dim, fmt.Sprintf("Raw Message: %s\n", err.Message)))
		if err.Cause != nil {
			sb.WriteString(color(style.Dim, fmt.Sprintf("Cause: %v\n", err.Cause)))
		}
	}
}
...
func formatGenericError(sb *strings.Builder, err error) {
	...
	if DevMode {
		sb.WriteString("\n")
		sb.WriteString(color(style.Dim, "--- Debug Info ---\n"))
		sb.WriteString(color(style.Dim, fmt.Sprintf("Error Type: %T\n", err)))
		sb.WriteString(color(style.Dim, fmt.Sprintf("Raw Error: %v\n", err)))
	}
}
